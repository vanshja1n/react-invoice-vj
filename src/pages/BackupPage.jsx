...
import { exportAllInvoices, importInvoices, clearAllInvoices, getAllInvoices } from '@/services/db';
...
export default function BackupPage() {
  ...
  const handleExport = async () => {
    try {
      const json = await exportAllInvoices();
      ...
      toast.success(`Exported ${invoices.length} invoice(s) as JSON`);
    } catch {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      const count = await importInvoices(text);
      await refresh();
      toast.success(`Imported ${count} invoice(s)`);
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      <PageHeader
        ...
        description="Manage your invoice data"
      />
      ...
      <div className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                  Your invoices are stored locally in your browser using IndexedDB. They persist
                  across sessions but are tied to this device and browser. Export a backup if you
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              ...
              {exportingZip ? `Exporting... ${zipProgress}%` : 'Export Backup (.zip)'}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Download all {invoices.length} invoice(s) as a JSON backup file for restoring later
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Restore invoices from a previously exported JSON file
            </CardDescription>
          </CardHeader>
          <CardContent>
            <input
        ...
        <Card className="border-destructive/30">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Permanently delete all invoices. This cannot be undone.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button
              ...
              size="sm"
            >
              <Trash2 className="h-4 w-4" /> Clear All Invoices
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <ConfirmDialog
        ...
        description={`This will permanently delete all ${invoices.length} invoice(s). Export a backup first if you want to keep your data. This action cannot be undone.`}
